...
TEST(s21ArrayTest, IteratorEndTest) {
  s21::array<int, 5> ar = {5, 6, 7, 8, 9}, arr(std::move(ar));
  int* endIter = arr.end();
  endIter--;
  endIter--;
  EXPECT_EQ(*endIter, 8);
...
TEST(s21ArrayTest, ArraySwapTest) {
  s21::array<std::string, 5> str_ = {"aa", "bb", "cc", "dd", "ee"}, str_arr;
  s21::array<std::string, 5> st = {"ff", "gg", "hh", "ii", "jj"}, str_arr2;
  str_arr = std::move(str_);
  str_arr2 = st;
...
TEST(s21ArrayTest, PairOfStringsTest) {
  s21::array<std::pair<std::string, std::string>, 5> m, mm(m);
